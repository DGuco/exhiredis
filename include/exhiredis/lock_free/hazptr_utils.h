...
#ifndef EXHIREDIS_HAZPTR_UTILS_H
#define EXHIREDIS_HAZPTR_UTILS_H
#include <atomic>
#include <thread>
...
public:
    void push(linked_list<Node> &l, bool may_be_locked) noexcept
    {
        ...
        uintptr_t oldval = head();
        while (true) {
            uintptr_t newval = reinterpret_cast<uintptr_t>(l.head());
            uintptr_t ptrval = oldval;
            uintptr_t lockbit = oldval & kLockBit;
            ...
            Node *ptr = reinterpret_cast<Node *>(ptrval);
...
#endif //EXHIREDIS_HAZPTR_UTILS_H
