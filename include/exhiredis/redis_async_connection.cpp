...
shared_ptr<CCommand> CRedisAsyncConnection::RedisvAsyncCommand(const char *format,
...
void CRedisAsyncConnection::RetryFailedOrWaitCommands()
{
...
void CRedisAsyncConnection::StartCheckTimer()
{
    if (m_pTimeOutEvent == nullptr) {
    ...
    tv.tv_usec = 0;  // 微秒
    //timer 事件
    evtimer_assign(m_pTimeOutEvent, m_pEventBase, &CRedisAsyncConnection::lcb_OnTimeoutCallback, (void *) this);
...
bool CRedisAsyncConnection::InitHiredis()
{
    ...
    if (redisAsyncSetDisconnectCallback(m_pRedisContext, &CRedisAsyncConnection::lcb_OnDisconnectCallback) != REDIS_OK) {
        SetConnState(enConnState::INIT_ERROR);
...
bool CRedisAsyncConnection::InitLibevent()
{
    m_pEventBase = event_base_new();
    if (nullptr == m_pEventBase) {
        HIREDIS_LOG_ERROR("Create event base error\n");
...
unsigned long CRedisAsyncConnection::GenCommandId()
{
    return ++m_cmdId;
}

//构建cmd
shared_ptr<CCommand> CRedisAsyncConnection::CreateCommand(const char *format, vector<shared_ptr<CCmdParam>> &param)
{
...
void CRedisAsyncConnection::lcb_OnDisconnectEventCallback(evutil_socket_t fd, short event, void *data)
{
    CRedisAsyncConnection *conn = (CRedisAsyncConnection *) data;
    if (conn != nullptr) {
        conn->SetConnState(enConnState::DISCONNECTED);
        HIREDIS_LOG_ERROR("Disconnected from redis");
        conn->ReConnect();
...
void CRedisAsyncConnection::lcb_OnTimeoutCallback(evutil_socket_t fd, short events, void *arg)
{
    CRedisAsyncConnection *conn = (CRedisAsyncConnection *) arg;
    if (conn != nullptr) {
